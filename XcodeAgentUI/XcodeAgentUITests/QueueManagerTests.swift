...
final class QueueManagerTests: XCTestCase {
  ...
  func testPauseRunningTicket() {
    ...
    XCTAssertEqual(
      sut.tickets.first?.status, .running,
      "Ticket should be running with available concurrency slots")
    sut.pauseTicket(id: id)
    XCTAssertEqual(sut.tickets.first?.status, .paused)
  }

  func testResumeTicketReEntersQueue() {
    sut.concurrencyLimits = ConcurrencyLimits(maxSonnet: 1, maxOpus: 0)
    sut.addTicket(makeTicket("TEST-1"))
    let id = sut.tickets.first!.id
    XCTAssertEqual(sut.tickets.first?.status, .running)
    sut.pauseTicket(id: id)
    XCTAssertEqual(sut.tickets.first?.status, .paused)
    sut.resumeTicket(id: id)
    let ticket = sut.tickets.first(where: { $0.id == id })
    XCTAssertNotEqual(ticket?.status, .paused, "Ticket should no longer be paused after resume")
    XCTAssertEqual(ticket?.status, .running)
  }

  func testResumeTicketStaysPendingWhenNoSlots() {
    sut.concurrencyLimits = ConcurrencyLimits(maxSonnet: 1, maxOpus: 0)
    sut.addTicket(makeTicket("TEST-1"))
    sut.addTicket(makeTicket("TEST-2"))
    let id1 = sut.tickets.first(where: { $0.ticketID == "TEST-1" })!.id
    sut.pauseTicket(id: id1)
    sut.concurrencyLimits = ConcurrencyLimits(maxSonnet: 0, maxOpus: 0)
    sut.resumeTicket(id: id1)
    let ticket = sut.tickets.first(where: { $0.id == id1 })
    XCTAssertEqual(ticket?.status, .pending)
    XCTAssertNil(ticket?.assignedAgent)
  }

  func testResumePendingTicketIsNoop() {
    sut.concurrencyLimits = ConcurrencyLimits(maxSonnet: 0, maxOpus: 0)
    sut.addTicket(makeTicket("TEST-1"))
    let id = sut.tickets.first!.id
    XCTAssertEqual(sut.tickets.first?.status, .pending)
