...
struct QueueView: View {
  ...
  private var filterBar: some View {
    HStack(spacing: 6) {
      QueueFilterChip(label: "All", count: queueManager.tickets.count, isSelected: filterStatus == nil) {
        filterStatus = nil
      }
      QueueFilterChip(
...
private struct QueueFilterChip: View {
  let label: String
  let count: Int
  let isSelected: Bool
  let action: () -> Void
    ...
    Button(action: action) {
      HStack(spacing: 4) {
        ...
        Text("\(count)")
          ...
          .padding(.vertical, 1)
          .background(isSelected ? .white.opacity(0.2) : Color.secondary.opacity(0.2))
          .clipShape(Capsule())
      }
      .font(.caption)
