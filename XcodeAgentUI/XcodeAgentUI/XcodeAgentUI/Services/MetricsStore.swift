...
/// SQLite-backed store for agent performance metrics
class MetricsStore: ObservableObject {
  @Published var runs: [TicketRun] = []
  @Published var agentStats: [AgentStats] = []
  @Published var dailyMetrics: [DailyMetric] = []
  @Published var timeRange: MetricsTimeRange = .week {
    didSet { reload() }
  }
  ...
  private func fetchRuns() -> [TicketRun] {
    ...
    if let start = timeRange.startDate {
      sql += " WHERE started_at >= \(start.timeIntervalSince1970)"
    }
  ...
  private func computeDailyMetrics(from runs: [TicketRun]) -> [DailyMetric] {
    ...
    return map.values.map { entry in
      var m = entry.metric
      let avg = entry.durations.isEmpty ? 0 : entry.durations.reduce(0, +) / Double(entry.durations.count)
      m.avgDuration = avg
      return m
  ...
  func seedDemoData() {
    ...
    for dayOffset in (0..<14).reversed() {
      ...
      for _ in 0..<runsPerDay {
        ...
        let run = TicketRun(
          id: UUID().uuidString,
          ticketID: "\(projects.randomElement()!.prefix(2).uppercased())-\(Int.random(in: 100...999))",
          ...
          inputTokens: model == "opus" ? Int.random(in: 20000...120000) : Int.random(in: 8000...50000),
          outputTokens: model == "opus" ? Int.random(in: 5000...40000) : Int.random(in: 2000...15000)
        )
        record(run)
      }
    }
