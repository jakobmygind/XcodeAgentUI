...
struct DiffReviewView: View {
  ...
  @StateObject private var review = CodeReview(sessionID: UUID(), ticketID: "")
  ...
  private var fileListPanel: some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      List(review.files, selection: $selectedFileID) { file in
        fileRow(file)
          .tag(file.id)
      }
      .listStyle(.sidebar)
  ...
  private func loadFromSession() {
    ...
    if bridgeWS == nil {
      bridgeWS = agentService.bridgeWS
    }
  }
  ...
  private func makeBridgeWS() -> BridgeWebSocket? {
    let ws = BridgeWebSocket(host: "localhost", port: agentService.bridgePort)
...
struct SideBySideDiffView: View {
  ...
  @ObservedObject var review: CodeReview
  ...
  private func inlineCommentsForLine(_ line: Int) -> some View {
    ...
    return ForEach(lineComments) { comment in
