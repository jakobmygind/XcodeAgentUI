...
public struct WorkloadView: View {
  public init() {}

    @Environment(AgentService.self) var agentService
    @State private var balancer = WorkloadBalancer()
...
struct AgentCoreCard: View {
    var agent: AgentWorker
    var balancer: WorkloadBalancer
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 4) {
                        Text(agent.name)
                            .font(.headline)
                        let modelLabel: String = agent.model == .opus ? "Opus" : "Sonnet"
                        Text("(\(modelLabel))")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
