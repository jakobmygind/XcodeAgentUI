...
public struct TicketAssignView: View {
  ...
  private var providerSection: some View {
    GroupBox("Ticket Source") {
      VStack(alignment: .leading, spacing: 12) {
        ...
        .onChange(of: assignment.provider) {
          if let provider = assignment.provider {
  ...
  private var isValid: Bool {
    assignment.provider != nil && !assignment.ticketID.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      && !assignment.project.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
  }
