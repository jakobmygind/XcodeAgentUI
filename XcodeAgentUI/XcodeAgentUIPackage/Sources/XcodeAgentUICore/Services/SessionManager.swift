...
public final class SessionManager {
  ...
  public var isConnectedAsHuman: Bool = false

  public var bridgeWS: BridgeWebSocket
  ...
  public init(bridgeWS: BridgeWebSocket) {
  ...
  public func startSession(ticketID: String, project: String, criteria: [String] = []) {
    let normalizedTicketID = ticketID.trimmingCharacters(in: .whitespacesAndNewlines)
    let session = AgentSession(ticketID: normalizedTicketID, project: project)
  ...
  public func approveRequest(_ request: ApprovalRequest) {
  ...
  public func denyRequest(_ request: ApprovalRequest) {
  ...
  private func processEnvelope(_ envelope: BridgeEnvelope) {
    ...
    guard shouldAccept(envelope: envelope, for: session) else { return }
  ...
  private func shouldAccept(envelope: BridgeEnvelope, for session: AgentSession) -> Bool {
    if envelope.type == "system" { return true }

    if let dict = envelope.payload.value as? [String: Any] {
      if let ticketID = dict["ticketId"] as? String, ticketID == session.ticketID {
        return true
      }
      if let target = dict["target"] as? String, target == session.ticketID {
        return true
      }
    }

    if envelope.from == "human" || envelope.from == "mission-control" || envelope.from == "bridge" {
      return true
    }
